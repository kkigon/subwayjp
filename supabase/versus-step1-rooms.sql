...
alter table public.jp_rooms drop constraint if exists jp_rooms_mode_check;
alter table public.jp_rooms add constraint jp_rooms_mode_check check (mode in ('all', 'custom'));
alter table public.jp_rooms drop constraint if exists jp_rooms_custom_lines_check;
alter table public.jp_rooms add constraint jp_rooms_custom_lines_check check (
  (mode = 'all' and custom_lines = '') or
  (mode = 'custom' and custom_lines ~ '^(G|M|H|T|C|Y|Z|N|F|A|I|S|E)(,(G|M|H|T|C|Y|Z|N|F|A|I|S|E))*$')
);

create index if not exists jp_rooms_created_idx on public.jp_rooms (created_at);
create index if not exists jp_rooms_public_lobby_idx on public.jp_rooms (last_active_at desc)
...
create policy jp_rooms_select_public on public.jp_rooms for select using (is_public = true);
